...
class CompileOptionsTest extends Specification {
    ...
    def setup()  {
        compileOptions = new BackwardsCompatibleCompileOptions(fileResolver)
    ...
    def "sets boot classpath via deprecated property"() {
        def resolvedBootstrapClasspath = Mock(FileCollectionInternal)
        when:
        compileOptions.bootClasspath = "unresolved1${File.pathSeparator}unresolved2"

        then:
        compileOptions.bootstrapClasspath == resolvedBootstrapClasspath
        1 * fileResolver.resolveFiles(["unresolved1", "unresolved2"]) >> resolvedBootstrapClasspath
        0 * _

        when:
        compileOptions.bootClasspath == "resolved1${File.pathSeparator}resolved2"

        then:
        1 * resolvedBootstrapClasspath.getAsPath() >> "resolved1${File.pathSeparator}resolved2"
        0 * _
    }
}
